...
OUTPUT_DIR = f"output_manhuavn_{current_time}"
...
def calculate_comprehensive_rating(story_data, sentiment_weight=0.4):
    ...
    try:
        ...
        views = extract_number(story_data.get('views', 0))
        followers = extract_number(story_data.get('followers', 0))
        
        # Xử lý rating và rating_count
        rating_str = story_data.get('rating', 'N/A')
        rating_count = extract_number(story_data.get('rating_count', 0))
        
        
        chapter_count = extract_number(story_data.get('chapters', 0))
        # # Xử lý chapters (chuỗi, cần đếm số lượng)
        # chapters_str = story_data.get('chapters', '')
        # if isinstance(chapters_str, str) and chapters_str.strip():
        #     try:
        #         chapters_list = eval(chapters_str)
        #         chapter_count = len(chapters_list)
        #     except:
        #         chapter_count = 1  # Default nếu không thể parse
        # else:
        #     chapter_count = 1  # Default nếu không có dữ liệu
        ...
        norm_views_efficiency = min(1.0, np.log10(views_per_chapter + 1) / np.log10(1000)) if views_per_chapter > 0 else 0
        ...
        view_score = (norm_views_total * 1.0) + (norm_views_efficiency * 2.5)  # Tổng: 3 điểm
        ...
        rating_score = (rating_value / 10.0) * 4 * 1  # Tổng: tối đa 4.5 điểm
        ...
        final_rating = min(10.0, max(0.0, final_rating * 1))
...
def analyze_comments_for_stories(db_path=DB_PATH, max_comments_per_story=None):
    ...
    try:
        ...
        stories_df = pd.read_sql_query('''
            SELECT 
                id, title, link, status, followers, views, 
                rating, rating_count, description, chapters, author
            FROM stories
        ''', conn)
        ...
        comment_counts = pd.read_sql_query('''
            SELECT story_title, COUNT(*) as comment_count
            FROM comments
        ...
        for i, (_, story) in enumerate(tqdm(list(stories_df.iterrows()), desc="Phân tích truyện")):
            ...
            comments_df = pd.read_sql_query('''
                SELECT id, user, content, sentiment, sentiment_score
                FROM comments
                WHERE story_title = ?
            ''', conn, params=(story_title,))
            ...
            sentiment_score = (positive_ratio * 9) - (negative_ratio * 4) + (neutral_ratio * 6)
...
def generate_comic_ratings(db_path=DB_PATH, max_comments_per_story=None, hide_analysis_method=True):
    ...
    try:
        ...
        for _, row in story_ratings_df.iterrows():
            ...
            # Xử lý chapters (chuỗi, cần đếm số lượng)            
            chapter_count = extract_number(row.get('chapters', 0))

            # chapters_str = row.get('chapters', '')
            # if isinstance(chapters_str, str) and chapters_str.strip():
            #     try:
            #         chapters_list = eval(chapters_str)
            #         chapter_count = len(chapters_list)
